...
%:-dynamic agent_ack/2. %Added by Ajitem for getting acks of agents send from this platform
:-dynamic agent_move_flag/1.
:-dynamic readcount/1.
...
make_platform_Tartarus_IITG(Ip,Port) :-                                         %% create platform at +port +Ip
   (platform_state(on)->                                                        %% To verify
        ...
        %assert(agent_ack(_dummy, '0')),                                           %% Added by Ajitem
        catch((
        ...
        mutex_create(tarpy_mutex),                                              %% Mutex for TarPy functionality of generic locking, Added by Ajitem
        mutex_create(tarpy_write_mutex),                                        %% Mutex for TarPy functionality of writing, Added by Ajitem
        mutex_create(tarpy_read_mutex)),                                         %% Mutex for TarPy functionality of reading, Added by Ajitem 
        Error,writeln(Error)),
...
close_tartarus :-
        ...
        mutex_destroy(tarpy_write_mutex),                                       %% Added by Ajitem
        mutex_destroy(tarpy_read_mutex),                                        %% Added by Ajitem
        mutex_destroy(tarpy_mutex),                                             %% Added by Ajitem 
...
agent_move(GUID,(Ip_other_end,Port_other_end)):-
        %retract_assert_tarpy(agent_move_flag(_), agent_move_flag('0')),
        platform_Ip(Ip1),
        get_ip(Ip),
        platform_port(Port),
...
handler(platform,(Ip,Port),send_ackm(GUID,_X)):- 
       ...
       (Il2=:=1,                                                            
        ...
        retractall(transit_token(GUID,_ListOfTokens)),
        retractall(transit_code(GUID,_AgentCode)),
        %retract_assert_tarpy(agent_ack(GUID, _), agent_ack(GUID, '1')), %Added by Ajitem for getting acks of agents send from this platform
        writeln('--------positive ack processed---------'),
        %retract_assert_tarpy(agent_move_flag(_), agent_move_flag('1')), %Added by Ajitem
        ttyflush,
        !;
        ttyflush,
        %writeln('--------- positive ACK received----------'),
        ...
        retractall(transit_token(GUID,_ListOfTokens)),
        retractall(transit_code(GUID,_AgentCode)),
        %writeln('Adding to agent_ack..'),
        %retract_assert_tarpy(agent_ack(GUID, _), agent_ack(GUID, '1')), %Added by Ajitem for getting acks of agents send from this platform
        %retract_assert_tarpy(agent_move_flag(_), agent_move_flag('1')),
        %writeln('Added to agent_ack..'),
        %writeln('--------positive ack processed---------'),
        ttyflush,
...
handler(platform,(_Ip,_Port),send_ackm(_GUID,_X)):- print_message(error,'handler(platform,(Ip,Port),send_ackm(GUID,_X)) failed'),
                                                %retract_assert_tarpy(agent_ack(GUID, _), agent_ack(GUID, '0')), %Added by Ajitem for getting acks of agents send from this platform                          %Added by Ajitem for getting acks of agents send from this platform
                                                %retract_assert_tarpy(agent_move_flag(_), agent_move_flag('1')),
                                                !.%------------------------------------------------------------------------------
...
handler(platform,(Ip,Port),send_nackm(GUID)):-
        ...
        %retract_assert_tarpy(agent_ack(GUID, _), agent_ack(GUID, '0')), %Added by Ajitem for getting acks of agents send from this platform
...
retract_assert_tarpy(X, Y):-
        % This is the writer process
        with_mutex(tarpy_write_mutex,(retractall(X), assert(Y))).

temp_retract_assert(X, Y):-
        mutex_create(temp_write_mutex),
        with_mutex(temp_write_mutex,(retractall(X), assert(Y))),
        mutex_destroy(temp_write_mutex).

%added for tarpy by Ajitem
mutex_acquire:-
        mutex_lock(tarpy_mutex).

mutex_release:-
        mutex_unlock(tarpy_mutex).


%added for tarpy by Ajitem
read_tarpy_entry:-
        % This is the reader process, but just the acquiring stage
        %writeln('In Read TarPy Entry'),
        with_mutex(tarpy_read_mutex,
        (
        %mutex_lock(tarpy_read_mutex),
        
        %writeln('Read Lock acquired'),
        %mutex_property(tarpy_read_mutex,status(Status1)), writeln('The status of read lock is ':Status1),
        % Update readcount to readcount + 1
        readcount(X),
        Y is X + 1,
        %write('X and Y are': X), writeln(' ': Y),
        temp_retract_assert(readcount(_), readcount(Y)),

        % Check if readcount is 1, if yes, wait for tarpy_write_mutex, else do nothing
        (Y = 1, writeln('trying to lock mutex'), mutex_lock(tarpy_write_mutex), writeln('Write mutex acquired'),mutex_property(tarpy_write_mutex,status(Status)), writeln('The status in entryyyyy ':Status) ; nothing)

        %writeln('Read Lock released')
        %mutex_unlock(tarpy_read_mutex),
        )),!.
        % After this statement, perform the read operation

%added for tarpy by Ajitem
read_tarpy_exit:-
        % This is the reader process, but just the release stage
        %writeln('In Read TarPy Exit'),
        ...
        % Check if readcount is 0, if yes, unlock tarpy_write_mutex, else do nothing
        (Y = 0, writeln('trying to unlock mutex'), mutex_property(tarpy_write_mutex,status(Status)), writeln('The status is ':Status), mutex_unlock(tarpy_write_mutex), writeln('
        Write mutex released'); writeln('This is failed') ,nothing)
...
read_tarpy_exit:-
        writeln('This is a failed predicate').
